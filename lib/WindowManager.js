...
function winChannel(windowID, name) {
...
class WindowManager {
    constructor(electron, modules, adaptorPath, options = {}) {
        ...
        this.winChannel = winChannel; // TODO clean this up
    ...
    mount({ windowInfo, editorPath, selector, callback = NOOP, loadArgs = [], parentEditor }) {
        if (!windowInfo) {
        ...
        const ipcSend = NOOP; // TODO this argument needs to be removed

        // Creates the editor itself, or gets the cached version
        const kwargs = { EditorClass, windowInfo, ipcSend, parentEditor };
        ...
        const afterMounted = () => {
            ...
            if (wasNewlyCreated) {
                ...
                editorInstance.on('ready', () => callback(editorInstance));
            }
        };
    ...
    mountEditor(browserWindow, editorInstance, selector, opts) {
        ...
        // Set up the meta info to be sent to initial mount
        const windowID = browserWindow.id;
        const prefix = this.getChannelPrefix(editorInstance);
        const mountPayload = { prefix, tagname, selector };

        editorInstance.load(() => {
            // Set up initial opts
            const initialOpts = JSON.stringify(editorInstance.getProps(true));
            mountPayload.opts = initialOpts;
            const channel = winChannel(windowID, 'mount:editor');
            browserWindow.webContents.send(channel, mountPayload);
            callback();
        }, ...loadArgs);
        return editorInstance;
    }

    createWindow(editorPath, callback = NOOP, ...loadArgs) {
        ...
        ipc.on(winChannel(windowID, 'mount:ready'), () => this.mount({
            windowInfo,
            editorPath,
            loadArgs,
            selector: '#main',
            callback: (editorInstance) => {
                ...
                browserWindow.show();
                browserWindow.webContents.send(winChannel(windowID, 'mount:hidesplash'));
                callback(editorInstance);
            },
        }));
        ...
        const querystring = `?windowID=${windowID}`;
        ...
        browserWindow.loadURL(`file://${mainPath}${querystring}`);
    ...
    registerEditorEvents(browserWindow, windowID, eventPairs) {
        for (const [eventName, methodName] of eventPairs) {
            browserWindow.on(eventName,
                () => this.editors.callMethodForWindow(windowID, methodName));
        }
    }
    ...
    */
    ...
    getIPCStream(windowInfo, path, subchannel) {
        ...
        const { windowID, browserWindow } = windowInfo;
        const channel = winChannel(windowID, `${path}:${subchannel}`);
    ...
    getChannelPrefix(editor) {
        const { windowID } = editor.windowInfo;
        return winChannel(windowID, `${editor.path}:`);
    }

    onChannel(editor, channel, callback) {
        const prefix = this.getChannelPrefix(editor);
        this.electron.ipcMain.on(`${prefix}${channel}`, callback);
    }

    sendOnChannel(editor, channel, args) {
        const { browserWindow } = editor.windowInfo;
        const prefix = this.getChannelPrefix(editor);
        browserWindow.webContents.send(`${prefix}${channel}`, ...args);
    }

    activateFirstWindow() {
        const windowID = Object.keys(this.windows)[0];
        const { browserWindow } = this.windows[windowID];
        browserWindow.show(); // force to bring to front & focus
    }
    ...
    destroyEditor(editor) {
        ...
        this.editors.callMethodRecursively(editor, 'onWindowClosed');
        this.editors.destroyEditor(editor);
    }
}

module.exports = WindowManager;
module.exports.winChannel = winChannel;
