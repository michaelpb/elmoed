...
class Editor {
    constructor(electron, modules, adaptorPath) {
    ...
    get_editor_class(typename) {
        const type_info = this.modules.types[typename];
        if (!type_info) {
            return null;
        }
    ...
    mount(window_info, wbobj, selector, callback = () => {}) {
        ...
        const type_info = this.modules.types[wbobj.typename];
        if (!type_info) {
    ...
    create_window(deck_path) {
        ...
        // Apply template
        const newText = this.getIndexContents({
    ...
    getIndexContents(ctx) {
        ...
        const templateText = fs.readFileSync(htmlPath);
